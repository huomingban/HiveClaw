...
flowchart LR
    ...
    subgraph file [持久化]
        F[".hiveclaw/im/session_map.json"]
    end
...
class SessionRouter:
    def __init__(self, workspace_dir):
        ...
        # 映射文件：.hiveclaw/im/session_map.json
...
flowchart TD
    ...
    subgraph storage [存储]
        GM[".hiveclaw/MEMORY.md"]
        CM[".hiveclaw/im/channel_id/MEMORY.md"]
    end
...
全局记忆（`.hiveclaw/MEMORY.md`）可以写这些内容：
...
频道记忆（`.hiveclaw/im/<channel_id>/MEMORY.md`）可以写：
...
旧 session 的文件（`.hiveclaw/sessions/im_xxx/`）还在磁盘上，不会被删除。只是 `session_map.json` 里的指向变了。如果你想恢复旧对话，可以手动把 JSON 里的值改回去。
