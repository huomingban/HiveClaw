...
`SessionStore` 把一次编程对话落盘到 `.hiveclaw/sessions/<session_id>/`，就像 RPG 里每个存档槽位一个文件夹：**meta.json** 记住你是谁、用的哪套模型、当前时间线末端在哪（`leaf_id`）；**context.jsonl** 像「剧情回顾」——按时间顺序一行一条消息，适合快速回放当前上下文；**session.jsonl** 则像「世界状态树」——每条消息节点带着 `parent_id`，从任意节点可以长出**平行分支**，因此同一会话文件里能表达树形历史，而不只是单列时间轴。
