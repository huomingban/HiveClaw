...
车间里还有一间**零件仓库**，也就是工作区下的 `.hiveclaw/` 目录。`WorkspaceResourceLoader` 像仓库管理员，专门读取 `settings.json`（运行参数与模型配置）、`prompt.md`（默认系统提示词）、`tools.json`（启用哪些内置工具）。这些文件若存在，就像给流水线贴上「默认配方」；而你在代码里传入的 `CreateAgentSessionOptions` 则像**领班下的临时工单**——多数时候工单可以覆盖仓库里的默认值，从而实现「同一套代码，不同项目不同行为」。
...
### `WorkspaceResourceLoader`：从 `.hiveclaw` 读入三类配置
...
class WorkspaceResourceLoader:
    def __init__(self, workspace_dir: str | Path) -> None:
        ...
        self.resource_root = self.workspace_dir / ".hiveclaw"  # 「仓库」根目录
